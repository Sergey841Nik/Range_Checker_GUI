...
##        print(height_table)
        self.tree = ttk.Treeview(self, columns=self.columns, height=height_table, show='headings', selectmode='browse', padding=20)
        self.tree.grid(columnspan=100, row=1, sticky='nsew')

        self.style = ttk.Style(self)
        self.style.configure('Treeview', rowheight=30)
        self.style.map('Treeview', background=[('selected', 'black')], foreground=[('selected', 'white')])
...
##                self.tree.insert('','end', values=k)
        ...
        self.main_menu_Btn_3.add_command(label='Удалить выделенную строку', command=self.delete_row)
        self.main_menu_Btn_3.add_command(label='Добавить новую строку', command=self.new_row)
        self.main_menu_Btn_3.add_command(label='Ввести значение', command=self.set_value_1)
...
##        self.win = WindTreeview('Vvod', '200x70+300+250')
        ...
        def save_edit():
            if self.column == '#1':
                self.tree.set(self.row, column=self.column, value=self.edit.get(1.0, 'end'))
            else:
    ...
    def save_file(self):
        ...
        self.data_save = [[i[j].replace('\n','').replace('\r','') for j in range(len(i))] for i in _save]
...
##        print(self.row, self.column)
    ...
    def format_column(self, stroka, widh_column):
        ...
        if font.measure(stroka) < widh_column: #sravnenie razmera stroki s shirinoy stolbca
            return stroka
        else:
            elements = stroka.split(',')
            lines = ['']
            for element in elements: #formirovanie spiska s elemetami dlinoq < shiriny stolbca
    ...
    def auto_format(self):
        col_widh = [self.tree.column(i)['width'] for i in self.tree['columns']] #spisok s shirinoq columns
        n = 0
    ...
    def motion_hendler(self, event):
        if self.tree.identify_region(event.x, event.y) == 'separator': #opredelyem mesto gde dvigaem mausom
            col = self.tree.identify_column(event.x)
            widh = self.tree.column(self.tree.identify_column(event.x))['width']  #opredelyem column i ego shirinu
