...
##    print(combo_index)
    ...
    def sort():
        ...
        wind.treb_znac = [data_pd_tr.loc[id] for id in data_pd_tr.index
            if data_pd_tr.loc[id]['Индекс'].split(',')[0] == index_sort.get()]
        value = [list(data_pd.loc[id]) for id in data_pd.index
...
##        print(wind.treb_znac)
    ...
    btn_grafik=ttk.Button(wind, text='График', command=lambda: grafik(valueX=wind.columns, valueY1=wind.value, valueY2=wind.treb_znac))
    ...
    def sort():
        if year_sort_from.get() > year_sort_to.get():
            ...
            for index in data_pd['Индекс']:
                data_pd_tr = pd.read_csv('required_range.csv', encoding='utf-8')
                value = [list(data_pd.loc[id]) for id in data_pd.index if data_pd.loc[id]['Индекс'].split(',')[0] == index.split(',')[0]]
                treb_znac = [data_pd_tr.loc[id] for id in data_pd_tr.index if data_pd_tr.loc[id]['Индекс'].split(',')[0] == index.split(',')[0]]
                serias_v = pd.Series(value[0], index=list(data_pd.columns))
                serias_s = pd.Series(ochenca_stab(value=value, treb_znac=treb_znac), index=list(data_pd.columns))
            ...
            columns_year_sort = [i for i in columns_year if year_sort_from.get()<=i<=year_sort_to.get()]
            wind.columns = columns_index.split() + columns_year_sort
            wind.value = [
                [data_pd_ocenka[column][id] for column in wind.columns
                    if year_sort_from.get()<=column<=year_sort_to.get() or column == 'Индекс'] for id in data_pd_ocenka.index
                                    ]
            wind.treeview()
    btn_sort=ttk.Button(wind, text='Сортировка', command=sort)
        ...
        if i != '-':
            ...
            for j in i:
                if treb_znac[0]['min'] == '-':
                    ...
                    if float(treb_znac[0]['min']) <= float(j) <= float(treb_znac[0]['max']):
                        ochenca_stab_promez.append('Ок')
                    else:
                        ochenca_stab_promez.append('НеОк')
...
def wind_menu():
    ...
    file_menu.add_command(label='Таблица с заданными значениями', command=table_treb_znach)
