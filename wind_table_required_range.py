...
class WindTreeview_required_range(WindTreeview):
    ...
    def treeview(self):
        self.tree = ttk.Treeview(
            self, columns=self.columns, height=20, show="headings", selectmode="browse"
        )
        ...
        self.main_menu_Btn_3.add_command(
            label="Удалить выделеную строку", command=self.delete_row
        )
        self.main_menu_Btn_3.add_command(label="Добавить строку", command=self.new_row)
        self.main_menu_Btn_3.add_command(
            label="Изменить значение", command=self.set_value_1
        )
    ...
    def save_file(self):
        ...
        return self.data_frame.to_csv(
            "required_range.csv", index=False, encoding="utf-8"
        )
